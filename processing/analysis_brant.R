# 0. Identification ----------------------------------
#Title:  Proportional odds test code for a research paper "Who justifies what? The role of the sense of injustice in the justifications of violence in the context of protest."
#Institution: Centro de Estudios de Conflicto y Cohesión Social (COES)
#Responsable: Research Assistant

# Executive Summary: This script contains the code to create the database needed to elaborate the analyses for "Who justifies what? The role of the sense of injustice in the justifications of violence in the context of protest.".
# Date: Sept 03, 2023

# 1. Load packages -----------------------------------

if (!require("pacman"))
  install.packages("pacman")  #if pacman es missing, install

pacman::p_load(tidyverse,
               sjmisc,
               sjPlot,
               summarytools,
               sjlabelled,
               car,
               haven,
               kableExtra,
               SciViews,
               ordinal,
               brant,
               MASS)

# 2. Load data --------------------------------------

procjv <-
  readRDS("input/data/proc/procjv.RDS") %>% dplyr::select(
    jv_est_2019_factor,
    jv_carab1_2019_factor,
    jv_carab2_2019_factor,
    sj_ceo_rec,
    sj_obrero_rec,
    sj_entrevistado_rec,
    ingreso,
    sexo_factor,
    educ_factor,
    ingreso,
    ingreso_satisfact_factor,
    sdo_indice,
    rwa_indice,
    trato_carab_factor
  ) %>% na.omit(.) # Proc data

# Re escalar ingresos
procjv$ingreso <- procjv$ingreso/300000 # Rescalar


# 3. Calculate binary models -------------------------------------------------------

# 3.1 Justification of students throwings rocks to police -------------------------

m1_jv_est <- polr(
  jv_est_2019_factor ~
    sj_ceo_rec +
    sj_obrero_rec +
    sj_entrevistado_rec,
  data = procjv, Hess = TRUE
  
)

m2_jv_est <- polr(
  jv_est_2019_factor ~
    sj_ceo_rec +
    sj_obrero_rec +
    sj_entrevistado_rec +
    ingreso +
    sexo_factor, #+
  #educ_factor +
  #ingreso_satisfact_factor,
  data = procjv, Hess = TRUE
  
)

m3_jv_est <- polr(
  jv_est_2019_factor ~
    sj_ceo_rec +
    sj_obrero_rec +
    sj_entrevistado_rec +
    ingreso +
    sexo_factor +
    #educ_factor +
    #ingreso_satisfact_factor +
    sdo_indice +
    rwa_indice,
  data = procjv, Hess = TRUE
  
)

m4_jv_est <- polr(
  jv_est_2019_factor ~
    sj_ceo_rec +
    sj_obrero_rec +
    sj_entrevistado_rec +
    ingreso +
    sexo_factor +
    #educ_factor +
    #ingreso_satisfact_factor +
    sdo_indice +
    rwa_indice +
    trato_carab_factor,
  data = procjv, Hess = TRUE
  
)




# 3.2 Justification of police repressing marches -------------------------

m1_jv_carab1 <- polr(
  jv_carab1_2019_factor ~
    sj_ceo_rec +
    sj_obrero_rec +
    sj_entrevistado_rec,
  data = procjv, Hess = TRUE
  
)

m2_jv_carab1 <- polr(
  jv_carab1_2019_factor ~
    sj_ceo_rec +
    sj_obrero_rec +
    sj_entrevistado_rec +
    ingreso +
    sexo_factor, #+
  #educ_factor +
  # ingreso_satisfact_factor,
  data = procjv, Hess = TRUE
  
)

m3_jv_carab1 <- polr(
  jv_carab1_2019_factor ~
    sj_ceo_rec +
    sj_obrero_rec +
    sj_entrevistado_rec +
    ingreso +
    sexo_factor +
    #educ_factor +
    #ingreso_satisfact_factor +
    sdo_indice +
    rwa_indice,
  data = procjv, Hess = TRUE
  
)

m4_jv_carab1 <- polr(
  jv_carab1_2019_factor ~
    sj_ceo_rec +
    sj_obrero_rec +
    sj_entrevistado_rec +
    ingreso +
    sexo_factor +
    #educ_factor +
    #ingreso_satisfact_factor +
    sdo_indice +
    rwa_indice +
    trato_carab_factor,
  data = procjv, Hess = TRUE
  
)

# 3.3 Justification of police evicting occupied schools -------------------------

m1_jv_carab2 <- polr(
  jv_carab2_2019_factor ~
    sj_ceo_rec +
    sj_obrero_rec +
    sj_entrevistado_rec,
  data = procjv, Hess = TRUE
  
)

m2_jv_carab2 <- polr(
  jv_carab2_2019_factor ~
    sj_ceo_rec +
    sj_obrero_rec +
    sj_entrevistado_rec +
    ingreso +
    sexo_factor, #+
  #educ_factor +
  #ingreso_satisfact_factor,
  data = procjv, Hess = TRUE
  
)

m3_jv_carab2 <- polr(
  jv_carab2_2019_factor ~
    sj_ceo_rec +
    sj_obrero_rec +
    sj_entrevistado_rec +
    ingreso +
    sexo_factor +
    #educ_factor +
    #ingreso_satisfact_factor +
    sdo_indice +
    rwa_indice,
  data = procjv, Hess = TRUE
  
)

m4_jv_carab2 <- polr(
  jv_carab2_2019_factor ~
    sj_ceo_rec +
    sj_obrero_rec +
    sj_entrevistado_rec +
    ingreso +
    sexo_factor +
    #educ_factor +
    #ingreso_satisfact_factor +
    sdo_indice +
    rwa_indice +
    trato_carab_factor,
  data = procjv, Hess = TRUE
  
)

# 4. See models --------------------------------------------------------------

sjPlot::tab_model(m1_jv_est,
                  m2_jv_est,
                  m3_jv_est,
                  m4_jv_est,
                  p.style = "stars",
                  show.ci = F)
sjPlot::tab_model(m1_jv_carab1,
                  m2_jv_carab1,
                  m3_jv_carab1,
                  m4_jv_carab1,
                  p.style = "stars",
                  show.ci = F)
sjPlot::tab_model(m1_jv_carab2,
                  m2_jv_carab2,
                  m3_jv_carab2,
                  m4_jv_carab2,
                  p.style = "stars",
                  show.ci = F)

# 5. Step models ---------------------------------------------------------------

step(m4_jv_est, direction = "backward")
step(m4_jv_carab1, direction = "backward")
step(m4_jv_carab2, direction = "backward")

# 6. Test -------------------------------------------------------------------

brant(m1_jv_est)
brant(m2_jv_est)
brant(m3_jv_est)
brant(m4_jv_est)

brant(m1_jv_carab1)
brant(m2_jv_carab1)
brant(m3_jv_carab1)
brant(m4_jv_carab1)

brant(m1_jv_carab2)
brant(m2_jv_carab2)
brant(m3_jv_carab2)
brant(m4_jv_carab2)

# 7. VIF -------------------------------------------------------------------

car::vif(m1_jv_est)
car::vif(m2_jv_est)
car::vif(m3_jv_est)
car::vif(m4_jv_est)

car::vif(m1_jv_carab1)
car::vif(m2_jv_carab1)
car::vif(m3_jv_carab1)
car::vif(m4_jv_carab1)

car::vif(m1_jv_carab2)
car::vif(m2_jv_carab2)
car::vif(m3_jv_carab2)
car::vif(m4_jv_carab2)

# 8. Diagnostics -------------------------------------------------------------------

sure::autoplot.polr(m1_jv_est)
sure::autoplot.polr(m2_jv_est)
sure::autoplot.polr(m3_jv_est)
sure::autoplot.polr(m4_jv_est)

sure::autoplot.polr(m1_jv_carab1)
sure::autoplot.polr(m2_jv_carab1)
sure::autoplot.polr(m3_jv_carab1)
sure::autoplot.polr(m4_jv_carab1)

sure::autoplot.polr(m1_jv_carab2)
sure::autoplot.polr(m2_jv_carab2)
sure::autoplot.polr(m3_jv_carab2)
sure::autoplot.polr(m4_jv_carab2)
